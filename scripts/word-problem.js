...
import { isHomophone } from './homophone.js';
  ...
  template: `
    <div>
      ...
      <div v-if="displayFormat === 'hear-type'" class="problem-display">
        <div class="instruction">Listen and type the word:</div>
        <Speak 
      ...
      <div v-if="displayFormat === 'hear-multiple-choice'" class="problem-display">
        <div class="instruction">Listen and select the word:</div>
        <Speak 
          :text="problem ? problem.answer : ''"
          :autoPlay="true"
          type="words"
      ...
      <div v-if="displayFormat === 'see-say'" class="problem-display">
        <div class="instruction">Say the word you see:</div>
  ...
  methods: {
    ...
    submitAnswer() {
      if (this.displayFormat === 'hear-type') {
        ...
        const correctAnswer = this.problem.answer.toLowerCase();
        // Check if answer matches exactly or is a homophone
        const isCorrect = answer === correctAnswer || isHomophone(answer, correctAnswer);
    ...
    handleRecognitionResult(result) {
      ...
      const normalizedAnswer = this.problem.answer.toLowerCase();
      
      // Check if the result matches the answer (exact match or word match)
      if (normalizedResult === normalizedAnswer || 
          normalizedResult.includes(normalizedAnswer) ||
          normalizedAnswer.includes(normalizedResult)) {
